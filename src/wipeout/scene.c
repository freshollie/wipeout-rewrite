...
void scene_pulsate_red_light(Object *obj) {
	uint8_t r = clamp(sins(system_cycle_time() * M_PI * 2) * 128 + 128, 0, 255);
...
void scene_move_oil_pump(Object *pump) {
	mat4_set_yaw_pitch_roll(&pump->mat, vec3(sins(system_cycle_time() * 0.125 * M_PI * 2), 0, 0));
}
...
void scene_update_aurora_borealis(void) {
	...
	for (int i = 0; i < 80; i++) {
		...
		if (aurora_borealis.grey_coords[i] != -2) {
			aurora_borealis.primitives[i]->color[0].r = (sins(coords[0] * phase) * 64.0) + 190;
			aurora_borealis.primitives[i]->color[0].g = (sins(coords[0] * (phase + 0.054)) * 64.0) + 190;
			aurora_borealis.primitives[i]->color[0].b = (sins(coords[0] * (phase + 0.039)) * 64.0) + 190;
		}
		if (aurora_borealis.grey_coords[i] != -2) {
			aurora_borealis.primitives[i]->color[1].r = (sins(coords[1] * phase) * 64.0) + 190;
			aurora_borealis.primitives[i]->color[1].g = (sins(coords[1] * (phase + 0.054)) * 64.0) + 190;
			aurora_borealis.primitives[i]->color[1].b = (sins(coords[1] * (phase + 0.039)) * 64.0) + 190;
		}
		if (aurora_borealis.grey_coords[i] != -1) {
			aurora_borealis.primitives[i]->color[2].r = (sins(coords[2] * phase) * 64.0) + 190;
			aurora_borealis.primitives[i]->color[2].g = (sins(coords[2] * (phase + 0.054)) * 64.0) + 190;
			aurora_borealis.primitives[i]->color[2].b = (sins(coords[2] * (phase + 0.039)) * 64.0) + 190;
		}

		if (aurora_borealis.grey_coords[i] != -1) {
			aurora_borealis.primitives[i]->color[3].r = (sins(coords[3] * phase) * 64.0) + 190;
			aurora_borealis.primitives[i]->color[3].g = (sins(coords[3] * (phase + 0.054)) * 64.0) + 190;
			aurora_borealis.primitives[i]->color[3].b = (sins(coords[3] * (phase + 0.039)) * 64.0) + 190;
		}
	}
}
