...
void droid_draw(droid_t *droid) {
	...
	int rf = sins(droid->cycle_timer) * 127 + 128;
	int gf = sins(droid->cycle_timer + 0.2) * 127 + 128;
	int bf = sins(droid->cycle_timer * 0.5 + 0.1) * 127 + 128;
...
void droid_update_intro(droid_t *droid, ship_t *ship) {
	...
	if (droid->update_timer < DROID_UPDATE_TIME_INTRO_3) {
		droid->acceleration.x = (-sins(droid->angle.y) * coss(droid->angle.x)) * 0.25 * 4096.0;
		droid->acceleration.y = 0;
		droid->acceleration.z = (coss(droid->angle.y) * coss(droid->angle.x)) * 0.25 * 4096.0;
		droid->angular_velocity.y = 0;
	}

	else if (droid->update_timer < DROID_UPDATE_TIME_INTRO_2) {
		droid->acceleration.x = (-sins(droid->angle.y) * coss(droid->angle.x)) * 0.125 * 4096.0;
		droid->acceleration.y = -140;
		droid->acceleration.z = (coss(droid->angle.y) * coss(droid->angle.x)) * 0.125 * 4096.0;
...
void droid_update_idle(droid_t *droid, ship_t *ship) {
	...
	droid->acceleration.x = (-sins(droid->angle.y) * coss(droid->angle.x)) * 0.125 * 4096;
	droid->acceleration.y = target_vector.y / 64.0;
	droid->acceleration.z = (coss(droid->angle.y) * coss(droid->angle.x)) * 0.125 * 4096;
