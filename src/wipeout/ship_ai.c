...
void ship_ai_update_race(ship_t *self) {
	...
	if (flags_not(self->flags, SHIP_FLYING)) {
		...
		self->angular_velocity.x = wrap_angle(-atan2f(track_target.y, xy_dist) - self->angle.x) * (1.0/16.0) * 30;
		self->angular_velocity.y = (wrap_angle(-atan2f(track_target.x, track_target.z) - self->angle.y) * (1.0/16.0)) * 30 + self->turn_rate_from_hit;
	}
	...
	// Ship is SHIP_FLYING
		...
		self->angular_velocity.y = wrap_angle(-atan2f(track_target.x, track_target.z) - self->angle.y) * (1.0/16.0) * 30;
	}
