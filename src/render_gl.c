...
static const char * const SHADER_POST_FS_CRT = SHADER_SOURCE(
	...
	void main(){
		...
		vec3 color;
		float x =  sins(0.3*time+uv.y*21.0)*sins(0.7*time+uv.y*29.0)*sins(0.3+0.33*time+uv.y*31.0)*0.0017;
		...
		float scanlines = clamp( 0.35+0.35*sins(3.5*time+uv.y*screen_size.y*1.5), 0.0, 1.0);
		
		float s = pow(scanlines,1.7);
		color = color * vec3(0.4+0.7*s);

		color *= 1.0+0.01*sins(110.0*time);
